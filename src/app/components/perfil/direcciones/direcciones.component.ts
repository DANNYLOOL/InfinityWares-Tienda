...
export class DireccionesComponent implements OnInit {
  ...
  registrar(registroForm:any){
    if(registroForm.valid){
      this.direccion.dni = this.direccion.dni.toUpperCase();
      ...
      const rfcValidationRegex = /^[A-Z]{4}[0-9]{6}[A-Z0-9]{3}$/;
      if (!rfcValidationRegex.test(this.direccion.dni.toUpperCase())) {
        iziToast.show({
          title: 'ERROR',
          titleColor: '#FF0000',
          color: '#FFF',
      ...
      if (!/^\d{10}$/.test(this.direccion.telefono)) {
        iziToast.show({
          title: 'ERROR',
          titleColor: '#FF0000',
          color: '#FFF',
